...
import {CustomButton, Error404, PayPalButton, ProposalForm, RatingForm, SkillList} from "../components";
    ...
    const fetchChat = async (jobData) => {
        try {
            ...
            if (user?.role === 'CUSTOMER') {
                chatData = await getChat(user?.userId, jobData?.proposals?.find(proposal => proposal?.status === "ACCEPTED").freelancer.userId, keycloak.token);
            } else {
                chatData = await getChat(user?.userId, jobData?.customer?.userId, keycloak.token);
            }
    ...
    const fetchJob = async () => {
        ...
        if (id) {
            try {
                ...
                if (keycloak.authenticated) {
                    keycloak.updateToken(10).then(async () => {
                        setChat(await fetchChat(response.data).data)
    ...
    const handleSendMessage = async () => {
        if (keycloak.authenticated) {
            keycloak.updateToken(10).then(async () => {
                ...
                await sendMessage(data, keycloak?.token);
                setMessage('')
                await fetchChat(job)
            });
        }
                        ...
                        <div className="max-w-3xl mx-auto">
                            <div className="bg-gray-100 border border-gray-300 rounded-lg p-4">
                                <div className="flex items-center border-b border-gray-200 pb-4 mb-4">
                                    <img src={job?.proposals?.find(proposal => proposal?.status === "ACCEPTED").freelancer.profilePicture} alt="User Avatar"
                                         className="w-10 h-10 rounded-full mr-2"/>
                                    <h2 className="text-lg font-medium">{user?.role === "CUSTOMER" ? job?.proposals?.find(proposal => proposal?.status === "ACCEPTED").freelancer.lastName + ' ' + job?.proposals?.find(proposal => proposal?.status === "ACCEPTED").freelancer.firstName : job?.customer?.lastName + job?.customer?.firstName}</h2>
                                </div>
                                ...
                                <div className="overflow-y-auto max-h-96 min-h-96">
                                    {chat?.messages ? (<>
                                            {chat?.messages?.map(message => (
                                                <>
                                                    {(message?.senderId === user.userId && user.role === 'CUSTOMER') ? (
                                                        <>
                                                    ...
                                                    ) : (message?.senderId === user.userId && user.role === 'FREELANCER') ? (
                                                        <div className="flex items-end justify-end mb-4">
                                                            <div className="bg-gray-200 rounded-lg p-2">
                                                                <p className="text-sm">{message?.message}</p>
                                                            </div>
                                                        ...
                                                        <div className="flex items-start mb-4">
                                                            <img src={job?.customer?.profilePicture} alt="User Avatar"
                                    ...
                                    ) : <p className="text-sm">Начтите диалог первыми!</p>}
                                </div>
                    ...
                    <div className="max-w-3xl mx-auto mt-8">
                        <div className="bg-gray-100 border border-gray-300 rounded-lg p-4">
                            ...
                            {job?.jobReference && (<>
                                    ...
                                    <a href={job?.jobReference} className="text-blue-500 hover:underline">Перейти на
                                        результаты
                                        работы</a>
                                </>
                            )}
