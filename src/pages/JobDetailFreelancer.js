...
import {CommentList, CustomButton, Error404, JobCard, ProposalForm, SkillList} from "../components";
...
const JobDetailFreelancer = () => {
    ...
    const [isProposalCreateDisplay, setIsProposalCreateDisplay] = useState(false);
    const [comments, setComments] = useState([]);

    const onProposalCreate = async (data) => {
        try {
            await keycloak.updateToken(10); // Обновляем токен
            if (keycloak.authenticated) { // Проверяем, аутентифицирован ли пользователь
                try {
                    ...
                    console.error("Ошибка при выполнении createProposal:", error);
    ...
    useEffect(() => {
        const isDisplay = user && job?.jobStatus === 'CREATED'
        const isProposalAlreadyExist = !!job?.proposals?.find(({freelancer}) => freelancer.userId === user.userId)
        setIsProposalCreateDisplay(isDisplay && !isProposalAlreadyExist);
    }, [user, job]);
            ...
            <div className='w-full flex flex-col md:flex-row gap-10'>
                <div className='w-full h-fit md:w-2/3 2xl:2/4 bg-white px-5 py-10 md:px-10 shadow-md'>
                  ...
                  {moment(job?.createdAt).fromNow()}
                    ...
                    {isProposalCreateDisplay && (
                            <CustomButton
                                title='Откликнуться сейчас'
                                onClick={() => setOpenProposalForm(true)}
                ...
                {(job?.customer?.userId !== user?.userId) ? (
                    ...
                    <>
                        <div className='w-full p-5 2xl:w-2/4 mt-20 md:mt-0'>
                            ...
                            {/*        <FreelancerCard proposal={proposal} onReject={onReject} onApprove={onApprove} onStartWork={onStartWork} jobId={job?.id}/>*/}
                            {/*    ))}*/}
                            {/*</div>*/}
                        </div>
                    </>
...
export default JobDetailFreelancer;